...
function App() {
  ...
  const [signinMessage, setSigninMessage] = useState("");
    // NEW: isAuthenticated state and useEffects
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(['loginState'], function(result) {
      if (result.loginState) {
        // If the user is logged in, set the isAuthenticated state to true
        setIsAuthenticated(true);
      } else {
        console.log('User is not authenticated according to loginState');
      }
    });
  }, []);
  useEffect(() => {
    if (isAuthenticated) {
      // If the user is authenticated, navigate to the Password Manager component
      ...
      salt: forge.util.encode64(salt),
        ...
        .then((response) => {
          if (response.data.message === "Registration successful") {
           // NEW: Set the login state in local storage
      ...
      .then((response) => {
        if (response.data.authenticated) {
          ...
          if (forge.util.encode64(hashedPassword) === response.data.hashedPassword) {
            ...
            console.log('encryptedMasterPassword:', encryptedMasterPassword);

            localStorage.setItem('email', email);
            setIsLoading(false); // Set loading to false when the request finishes
                    // Store data in chrome.storage.local before navigating
        chrome.runtime.sendMessage({ message: "login" }); 
