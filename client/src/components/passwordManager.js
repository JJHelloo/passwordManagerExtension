...
function PasswordManager() {
  ...
  useEffect(() => {
    if (!encryptedMasterPassword) {
      console.log('encryptedMasterPassword is missing in state. Checking chrome.storage.local...');
      
      chrome.storage.local.get(['encryptedMasterPassword', 'encryptionKey', 'salt', 'iv'], function(result) {
        if (!result.encryptedMasterPassword) {
          console.log('encryptedMasterPassword is also missing in chrome.storage.local. Redirecting to root...');
          navigate("/");
        } else {
          const masteredPassword = decryptMasterPassword(result.encryptedMasterPassword, result.encryptionKey, result.salt, result.iv);
  ...
  useEffect(() => {
    Axios.get(`${process.env.REACT_APP_API_URL}/checkAuthentication`, {
      withCredentials: true,
      headers: {
        'Authorization': `Bearer ${sessionId}`, // Provide the session token in the Authorization header
      },
    })
      .then((response) => {
        if (!response.data.authenticated) {
          console.log('Redirecting to root because user is not authenticated');
          navigate("/");
        }
      })
      .catch((error) => {
  ...
  const handleLogout = () => {
    Axios.post(`${process.env.REACT_APP_API_URL}/logout`, {}, { withCredentials: true })
      .then(() => {
        ...
        localStorage.removeItem('sessionId');  // Remove the session ID
      // Remove data from chrome.storage.local
      chrome.storage.local.remove(['encryptedMasterPassword', 'encryptionKey', 'salt', 'iv'], () => {
        navigate("/");
      });
      })
      .catch(err => { 
